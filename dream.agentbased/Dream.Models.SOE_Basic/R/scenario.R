...
pplot = function(zz, sMain, ylab="Relative change", s_ylim=c(1,1))
{
  
  col = rgb(0.5,0.7,0.7)
  mx=max(zz$up)
  mn=min(zz$lo)
  plot(zz$Time/12, hpfilter(zz$up, 900000), type="l", col=bcol, ylim=c(s_ylim[1]*mn,s_ylim[2]*mx), xlab="Year", cex.axis=0.8, 
...
#  polygon(c(zz$Time/12, rev(zz$Time/12)), c(zz$lo, rev(zz$up)), col=bcol, lty=0)
  lines(zz$Time/12, hpfilter(zz$lo, 900000), col=bcol)
  polygon(c(zz$Time/12, rev(zz$Time/12)), c(hpfilter(zz$lo, 900000), rev(hpfilter(zz$up, 900000))), col=bcol, lty=0)
